    ...
    <div className="flex flex-col gap-2 pt-2 pl-14 w-full">
      <div className="relative">
        ...
        <input 
          ...
          disabled={loading}
          className="w-full pl-9 pr-4 py-2 bg-white border border-cyan-200 text-slate-800 rounded-xl text-sm font-medium focus:ring-2 focus:ring-cyan-600 focus:outline-none transition-all shadow-sm"
        />
      </div>
      {!loading && filtered.length > 0 && (
        <div className="bg-white rounded-xl border border-cyan-100 shadow-sm overflow-hidden flex flex-col mt-1">
          {filtered.map(ind => (
            <button 
              key={ind}
              onClick={() => onSelect(ind)}
              className="text-left px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-cyan-50 hover:text-cyan-800 transition-colors border-b border-slate-50 last:border-b-0"
            >
              {ind}
            </button>
          ))}
...
function UnitSearchList({ options, selected, onToggle, disabled }: { 
  ...
  return (
    <div className="bg-white p-2 rounded-xl border border-cyan-100 shadow-sm flex flex-col">
      <div className="relative mb-2 shrink-0">
        <Search className="absolute left-2.5 top-2 w-4 h-4 text-cyan-700" />
        <input 
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          ...
          disabled={disabled}
          className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 text-slate-800 rounded-lg text-sm focus:ring-2 focus:ring-cyan-600 focus:outline-none transition-all disabled:opacity-50"
        />
      </div>
      <div className="max-h-48 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
        {filtered.map((opt) => {
          ...
          return (
            <label 
              key={opt.id} 
              className={`flex items-center space-x-3 p-2 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-cyan-50' : 'hover:bg-slate-50'} ${disabled ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors shrink-0 ${isSelected ? 'bg-cyan-700 border-cyan-700' : 'border-slate-300'}`}>
...
export default function AquaBot() {
  ...
  return (
    <div className="w-full max-w-lg h-[650px] flex flex-col bg-white rounded-3xl shadow-2xl overflow-hidden border border-cyan-100 relative">
      {/* Header */}
      <div className="bg-linear-to-r from-cyan-800 to-emerald-700 p-5 flex items-center space-x-3 text-white shrink-0 shadow-sm relative z-10">
        <div className="p-2 bg-white/20 rounded-full shadow-inner">
          <Droplets className="w-6 h-6 text-white" />
        </div>
        <div>
          <h3 className="font-bold text-lg leading-tight">AquaBot</h3>
          <p className="text-sm font-medium text-cyan-50 opacity-90">Issue Reporting System</p>
        </div>
      </div>

      {/* Messages Layout */}
      <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-slate-50 relative">
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[radial-gradient(circle_at_center,var(--tw-gradient-stops))] from-cyan-600 via-transparent to-transparent"></div>
        ...
        {messages.map((msg) => (
          <div key={msg.id} className="relative z-10 flex flex-col space-y-2">
            <div className={`flex items-end space-x-3 ${msg.sender === 'user' ? 'flex-row-reverse space-x-reverse' : 'flex-row'}`}>
              <div className={`p-2.5 rounded-full shrink-0 shadow-sm ${msg.sender === 'user' ? 'bg-cyan-100 text-cyan-800' : 'bg-cyan-700 text-white'}`}>
              ...
              >
                {msg.isImage ? (
                  <div className="flex flex-col space-y-2">
                    <span className="text-xs opacity-75 flex items-center"><ImageIcon className="w-3 h-3 mr-1"/> Uploaded Attachment</span>
                    {msg.text.startsWith('data:video') ? (
                      <video controls className="max-w-full rounded-md max-h-48 object-cover border border-black/10">
                        <source src={msg.text} />
                      </video>
                    ) : (
              ...
              <div className="flex flex-wrap gap-2 pt-2 pl-14">
                {msg.options.map((option, idx) => (
                  <button
                    ...
                    className="px-4 py-2 bg-white border border-cyan-200 text-cyan-800 rounded-full text-sm font-medium hover:bg-cyan-50 focus:ring-2 focus:ring-offset-1 focus:ring-cyan-600 transition-colors disabled:opacity-50"
                  >
                    {option}
                  </button>
                ))}
              ...
              <div className="flex flex-col gap-2 pt-2 pl-14 w-full max-w-sm">
                ...
                {msg.id === messages[messages.length - 1].id && !isDone && (
                  <button
                    ...
                    className="mt-2 self-start px-5 py-2 bg-linear-to-r from-cyan-800 to-emerald-700 text-white rounded-full text-sm font-medium hover:shadow-md focus:ring-2 focus:ring-offset-1 focus:ring-cyan-600 transition-all disabled:opacity-50"
                  >
                    Confirm Selection
                  </button>
                )}
        ...
        <div className="p-4 bg-white border-t border-slate-100 shrink-0 relative z-10 transition-all">
          <form onSubmit={onSubmit} className="flex items-center space-x-2">
            ...
            <input
              ...
              disabled={isDone}
              className="flex-1 px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-full text-[15px] text-slate-900 focus:outline-none focus:ring-2 focus:ring-cyan-600 focus:border-transparent transition-all disabled:opacity-50 placeholder:text-slate-500 font-medium"
            />

            <button
              type="submit"
              disabled={(!inputValue.trim() && !isAwaitingPhoto) || isDone}
              className="p-3.5 bg-linear-to-r from-cyan-800 to-emerald-700 text-white rounded-full hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-600 disabled:opacity-50 transition-all duration-200 transform shrink-0 active:scale-95"
            >
